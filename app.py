...
# Step 2: 打席・イニング・打点・盗塁判定用プロンプト（四角結線×得点丸判定 ＆ 全マス完全走査）
DETAILS_PROMPT = """
### 役割
...
### 【最重要】「四角（◇）結線 × 中の丸数字（得点）」の厳密判定ルール
早稲田式スコアでは「本塁打」と「安打・四球等で出塁した後の得点（ホームイン）」の書き方が明確に異なります。以下の基準を厳格に適用してください：

1. **【本塁打（ホームラン）の条件】**:
   - ダイヤモンド外枠の4辺（本塁→一塁→二塁→三塁→本塁）すべてが赤線で完全に一周囲まれている（◇の四角形が完成している）。
   - かつ、その中央に「丸の中に数字（得点丸数字）」が書かれている。
   - ※外枠の◇が4辺すべて赤線で閉じていない限り、中央に丸数字があっても絶対に本塁打にしてはいけません。
...
2. **【単打・四球・凡打 ＋ 得点（ホームイン）の条件】**:
   ...
   - ※中央の得点丸に惑わされて、単打や四球を本塁打や2塁打・3塁打と誤認してはいけません。
        ...
        if st.button(f"AIで全{len(uploaded_files)}試合を高精度一括解析する", type="primary"):
            ...
            for idx, f in enumerate(uploaded_files):
                ...
                try:
                    ...
                    status_text.text(f"【{idx+1}/{len(uploaded_files)}】{f_name} の全イニング打席をサボらず完全走査中...")
                    res_details = client.models.generate_content(
                        model="gemini-3.6-flash",
                        contents=[
                            types.Part.from_bytes(data=raw_bytes, mime_type="image/jpeg"),
                            types.Part.from_bytes(data=enhanced_red_bytes, mime_type="image/jpeg"),
                            f"確定選手名簿:\n{roster_data}\n\n上記選手枠に基づき、スコアブックの1回〜7回の全打席詳細、打点、盗塁を四角結線と得点丸を厳密に区別して判定してください。"
                        ],
                        config=types.GenerateContentConfig(
