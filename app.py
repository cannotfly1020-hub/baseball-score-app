...
def render_admin_view():
    ...
    with col_img:
        ...
        zoom_val = st.slider("🔍 拡大率", min_value=100, max_value=300, value=140, step=20, format="%d%%")
        box_height = 280 if is_mobile_sticky else 500
        viewer_html = f"""
        <div style="width:100%; height:{box_height}px; overflow:auto; border:2px solid #555; border-radius:6px; background-color:#222; text-align:center;">
    ...
    with col_grid:
        ...
        for idx, (p_tab, player) in enumerate(zip(player_tabs, current_players)):
            with p_tab:
                ...
                st.markdown(f"#### **【{order_val}番】 #{u_num_init or '-'} {p_name_init} {'（交代）' if is_sub else '（先発）'}**")
                ...
                with st.form(key=f"form_player_{selected_match_file}_{idx}"):
                    ...
                    for i_idx, inn_str in enumerate(["1", "2", "3", "4", "5", "6", "7"]):
                        with inn_cols[i_idx]:
                            st.markdown(f"<div class='inning-badge'>◇ {inn_str}回</div>", unsafe_allow_html=True)
                    ...
                    st.write("")
                    submitted = st.form_submit_button("💾 この選手を保存", use_container_width=True)
                    if submitted:
