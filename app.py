...
- 右下の辺のみ赤線（一塁到達）: 単打（ヒット / hits）
- 一塁から二塁（真上頂点）まで連続した赤線: 二塁打（doubles）
- 一塁〜二塁〜三塁（左端頂点）まで連続した赤線: 三塁打（triples）
- 本塁まで四角を赤線で一周囲んでいる: 本塁打（homeruns）
※黒鉛筆の記号があっても、赤ペンでダイヤモンドが結線されている場合は安打の記録を最優先してください。

【選手交代と打席の割り当てルール（重要）】
- 打順欄に上下二段で選手名が書かれている場合、上段が「先発選手」、下段が「途中出場・代打選手」です。
- 打順マス目付近に「赤ペンで代打（またはPH）」や選手名が追記された打席から、下段の交代選手に切り替えて打席・成績を集計してください。
- 交代前のイニング・打席は上段の先発選手の記録として集計してください。

【括弧書き数字「(数字)」の解釈（重要）】
- マス目内の「(6)」「(8)」「(9)」等の括弧付き数字は、「その打順の選手の打撃・プレーによって進塁したこと」を示す進塁責任打者の記録です。
- 打者本人の打撃結果や守備位置コードではないため、混同せず進塁・得点・打点の補助情報として処理してください。

【スコア記号の標準解釈】
- 「K」「Ⓚ」: 三振（strikeouts）
- 「四」「B」: 四球（walks）
- 「DB」「死」: 死球（dead_ball）
- 「3A」「1A」などの「数字+A」: 内野ゴロ凡打（アウト）
- 「4-3」「6-4-3」などのハイフン表記: 内野ゴロ送球アウト / 併殺打
- 丸囲みの数字（①, ②, ③）: そのイニングのアウトカウント
- 「wp」: 暴投、「pb」: 捕逸、「S」: 盗塁

【確信度と判定の柔軟性】
- 上記の標準的な早稲田式記号や赤線の規則に合致している場合は、むやみに「要確認」にせず確定値として数値をカウントしてください。
- 本当にインク潰れなどで判別不可能な箇所のみ null や "UNREADABLE" とし、is_unreadable を true にしてください。
...
[
  {
    ...
    "rbi": 打点(数値またはnull),
    "runs": 得点(数値またはnull),
    "highlight": "その試合の印象的なプレー（例: 4回裏の豪快なランニング本塁打、気迫の代打適時打など）",
