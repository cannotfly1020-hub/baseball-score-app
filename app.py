...
# Step 2: 打席・イニング・打点・盗塁判定用プロンプト（打順の連続性検証・赤線対照表）
DETAILS_PROMPT = """
### 役割
...
### 安打・凡打・四死球の厳格判定基準（対照表）
中央のひし形（ダイヤモンド）の「赤ペンの結線」を最優先で確認してください。

- **単打**: 右下の辺（本塁→一塁）のみに明確な赤線がある。※黒鉛筆で「8」「7」「3A」などの打球方向や凡打記号が書かれていても、赤線で一塁へ結線されていれば【単打】とする。
- **2塁打**: 一塁から二塁（真上の頂点）まで連続した赤線がある。
- **3塁打**: 一塁〜二塁〜三塁（左端）まで連続した赤線がある。
- **本塁打**: 赤線で四角形（ダイヤモンド）を一周囲んでいる。
- **凡打**: 赤線が一切引かれておらず、黒字で「3A」「4-3」「K」「8」などのアウト記号、または丸囲み数字（①②③）がある。
- **四球 / 死球**: 赤線結線がなく、「四」「B」または「死」「DB」と明記されている。
- **犠打**: 「□」「サ」などの犠打記号がある。
- **盗塁（stolen_bases）**: ダイヤモンドの進塁線付近に「S」または「ㄭ」の記号がある個数をカウント。※盗塁による進塁線を安打の赤線と混同しないこと。
- **打点（rbi）**: 適時打（タイムリー）のマス目隅に小さく書かれた数字、または明確な得点打から計算。

### 括弧書き数字「(数字)」の除外（最重要）
- マス目内の「(6)」「(8)」などの括弧付き数字は「進塁責任打者」の記録です。打者本人の打撃結果や打点コードではありません。無視してください。
